...
export class ProviderService {
  ...
  async create(user, createProviderDto: CreateProviderDto) {
    const {logo,country, state, city, ...restInputs} = createProviderDto
    console.log(logo)
    if (logo) {
      await this.uploadImage(logo, true)
    }
    const newAddress = await this.addressService.create({country, state, city})
    const newProvider = await this.providerRepository.create({...restInputs, ...(logo ? { logo: logo.originalname } : {})})
  ...
  findOne(id: number) {
    return this.providerRepository.findOne({where: {id}, relations: ['address', 'user']})
  }
  findOneByName(name: string) {
    return this.providerRepository.findOne({where: {name}, relations: ['address', 'user']})
  }

  async update(id: number, updateProviderDto: UpdateProviderDto) {
    const provider = await this.findOne(id);
    if (!provider) throw new HttpException("Provider not found", HttpStatus.BAD_REQUEST);
    const { logo, country, city, state, ...restInputs } = updateProviderDto;
    await this.addressService.update(provider.address?.id, { country, state, city });
    if (logo) {
      await this.uploadImage(logo, true);
    }
